...
CONLLU_PATH = "../../data/nkjp/resources/NKJP_UD/NKJP1M-UD.conllu"
XML_DIR = "../../data/nkjp/resources/NKJP/"
PDB_TEST = "../../data/nkjp/resources/UD_Polish-PDB/pl_pdb-ud-test.conllu"
PDB_DEV = "../../data/nkjp/resources/UD_Polish-PDB/pl_pdb-ud-dev.conllu"

with open("../../data/nkjp/static/pl_morph.json") as f:
...
def extract_entities_from_sentence(ner_sent):
    ...
    for i, seg in enumerate(segs):
        ...
        targets = [
            ptr.get("target") for ptr in seg.xpath("x:ptr", namespaces=NAMESPACES)
        ]
        orth = seg.xpath("x:fs/x:f[@name='orth']/x:string", namespaces=NAMESPACES)[
            0
        ].text
        ner_type = seg.xpath("x:fs/x:f[@name='type']/x:symbol", namespaces=NAMESPACES)[
            0
        ].get("value")
        ner_subtype_node = seg.xpath(
            "x:fs/x:f[@name='subtype']/x:symbol", namespaces=NAMESPACES
        )
        if ner_subtype_node:
...
def resolve_entities(entities):
    ...
    resolved_targets = {
        entity_id: resolve_entity(entity, entities)
...
def get_seg_tag(seg):
    interp = seg.xpath(
        "x:fs/x:f[@name='disamb']/x:fs/x:f[@name='interpretation']/x:string",
    ...
    for par_key in subfolder_entities:
        ...
        for sent_key in par_sents:
            ...
            for entity in sent_entities:
                entity["targets"] = sorted(
                    list(set([id_to_position[target] for target in entity["targets"]]))
                )
...
def reanalyze_complex_verb(verb, sub_tokens):
    ...
    verb_xpos = [
        f for f in verb_xfeats if morph_data["VAL2ATTR"][f] == "Part of Speech"
    ][0]
    for sub_token in sub_tokens:
        if sub_token["head"] == verb["id"]:
            ...
            if sub_token["deprel"] == "aux:clitic":
                x_person = [
                    f for f in sub_token_xfeats if morph_data["VAL2ATTR"][f] == "person"
                ][0]
    ...
    sorted_verb_xpos = sorted(
        verb_xfeats,
        key=lambda feat: morph_data["ATTRIBUTE_ORDER"][morph_data["VAL2ATTR"][feat]],
    )
...
def retokenize_conllu_sent(sent):
    ...
    for i, token in enumerate(sent):
        ...
        token_xpos = [
            f for f in token_xfeats if morph_data["VAL2ATTR"][f] == "Part of Speech"
        ][0]
        if token_xpos in ["praet", "cond", "winien"]:
            if "Person" not in token["feats"]:
                ...
                sorted_token_xfeats = sorted(
                    token_xfeats,
                    key=lambda feat: morph_data["ATTRIBUTE_ORDER"][
                        morph_data["VAL2ATTR"][feat]
                    ],
                )
...
def load_xml_nkjp():
    ...
    for subfolder in tqdm(
        [
            ...
            if os.path.isdir(os.path.join(XML_DIR, name))
        ]
    ):
        (
            subfolder_to_entities[subfolder],
            subfolder_to_lens[subfolder],
        ) = extract_entities_from_subfolder(subfolder)
        seg_id_to_par_id[subfolder], seg_id_to_index[subfolder] = extract_seg_ids(
            subfolder
        )
    return subfolder_to_entities, seg_id_to_par_id, seg_id_to_index, subfolder_to_lens


def assign_named_entities_to_conllu_sent(
    ...
    for sents in par_list:
        ...
        for sent_i, sent in enumerate(sents):
            ...
            for token in sent:
                ...
                if token["feats"]:
                    feats = "|".join(
                        [f"{attr}={val}" for attr, val in token["feats"].items()]
                    )  # filter attrs here?
                else:
                    feats = ""
                morphs.append(feats)
...
def process_nkjp(
    ...
    return (
        ...
        convert_to_doc_objs(test_pars, doc_test_bin, nlp, categoriesMapping),
    )
